...
def resnet_blockv1(input_data, mid_channel, output_channel, trainable, name):
...
def resnet_blockv2(input_data, mid_channel, output_channel, trainable, name):
    short_cut = input_data
    with tf.variable_scope(name):
        input_data = convolutional(input_data, (1, mid_channel), trainable, 'conv1')
        input_data = convolutional(input_data, (3, output_channel), trainable, 'conv2')
        output_data = input_data + short_cut
    return output_data


def route(previous_output, current_output, name):
    with tf.variable_scope(name):
...
def upsample(input_data, name, method="deconv"):
    ...
    if method == "resize":
        with tf.variable_scope(name):
            ...
            output = tf.img.resize_nearest_neighbor(input_data, (input_shape[1] * 2, input_shape[2] * 2))
