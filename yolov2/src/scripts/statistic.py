...
class statistic(object):
    def __init__(self):
        ...
        self.class_list        = cfg.YOLOv2.CLASSES_LIST
    ...
    def statist_all_labels(self):
        ...
        img_anchors = self.analysis_box3d(total_dimensions, total_types)
        loss_scale = self.analysis_types(total_types, img_anchors)
    ...
    def calc_mean_dxdy(self, w, l):
        ww = int(w / (cfg.IMG.X_RESOLUTION * 2))
        ll = int(l / (cfg.IMG.X_RESOLUTION * 2))
        dx_sum = 0
        dy_sum = 0
        for i in range(ww):
            dx_sum += i
    ...
    def analysis_types(self, types, img_anchors):
        types_set = set(types)
        ...
        img_types_num = types_num_array * img_anchors[:, 1] * img_anchors[:, 2]
        img_ratio = 1 / ( img_types_num / np.sum(img_types_num))
        img_scale = img_ratio / img_ratio[0]
        print(img_ratio, img_scale)
